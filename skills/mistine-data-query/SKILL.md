...
已加入云视频采集的腾讯 ADQ 素材审核数据关联，可按已授权数据访问路径查询审核状态、具体拒审原因、广告账户、平台素材 ID、创意/组件、视频版本、当前/历史记录和刷新时间。字段、关联表和覆盖说明见 [审核记录查询说明](references/queries.md#云视频采集的-adq-审核记录)。

- 数据能力已落在公网 ADQ 数据库，映射任务完成后自动同步；当前 `mistine_data_query.py` 和普通查询 API 尚未开放专用卡审入口。不得编造 `adq-audits` 命令，或声称 `adq-videos` 已返回审核字段。只有 API Key、没有已授权数据库访问路径的客户端，应明确说明入口尚未开放。
- 数据库源为 `sucaiwang_audit_records` 的腾讯 ADQ 记录，已同步至 ADQ 库 `cloud_adq_audit_records`。按账户素材查询 `video_asset_cloud_audit_details`；素材消耗报表关联一行摘要 `video_asset_cloud_audit_summary`，避免消耗翻倍。
- “素材卡审/拒审”优先查平台审核记录；云视频内部“待审核”不能代替 ADQ 审核状态。仅显示平台实际返回的原因，不推测长期审核中的原因。
- 输出区分当前和历史、账户和版本；未查到记录或覆盖未知不等于审核通过。当前拒审也仅代表相应广告/创意/组件的记录，不扩大为所有计划均不可投放。
...
- ADQ `account_id` 是实际消耗账户，`owner_account_id` 是素材官方归属账户，两者可能不同。同一视频可以在多个子账户产生消耗，标题及归属资料可能只在其他账户可见。标题为空时先核对 `mapping --adq-video-id` 与其他可访问账户的同一官方身份，不得直接判为素材不存在或云视频未匹配。后台按 video_id、media_id、source_material_id 别名及归属／签名证据补齐资产；普通 Skill 只查询结果，不自行抓网页、连接数据库或重跑采集。云视频成功／完成推送记录优先：先核对实际子账户，再在官方身份确认后核对归属账户。多个推送候选冲突时保留歧义，不用随机、标题或分组覆盖。跨账户补齐标题或映射不改变消耗归属，不重复计入归属账户。
...
- 多个可加指标同时按相同比例放大时，核对服务端事实行唯一键及 JOIN 前后行数、金额。按直播间名称关联资料表尤其要验证右表唯一性；本地代码中的风险不能代替线上数据证据。ROI 相同不能证明金额正确。
- 最终确认需取得原始事实汇总与关联后结果，解释重复或漏计记录并保留证据。计划明细是另一条事实链，不能默认覆盖完整；消耗为空必须单列，不能当零，也不能仅凭刷新较晚就认定更准确。
- 只使用接口实际实现的筛选参数。当前本地 0.1.10 的素材接口代码未实现 `order-class`；计划素材接口支持。服务器版本可能变化，应核对实现和返回证据，不能用被忽略的参数宣称已拆分口径。
- 普通查询继续使用只读 API。用户已明确要求故障诊断、最终核验时，在现有授权内继续只读排查；不要把普通查询流程当成必须重新索要许可的理由。诊断不自动授权部署或修改生产数据。若没有原始记录访问能力，说明实际连接/权限错误、已尝试的路径和缺失证据，金额标记“未最终确认”，不要声称核验完成。
